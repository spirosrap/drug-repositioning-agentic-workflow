...
# Load .env early so dependent modules can read env vars on import.
load_dotenv(".env")
...
class DataMiningAgent(BaseAgent):
    ...
    def run(self, target: str, disease: str) -> Tuple[List[DrugCandidate], str]:
        ...
        ot_data = query_opentargets(target)
        known_drugs = ot_data.get("known_drugs", []) if isinstance(ot_data, dict) else []
        logger.info(f"  Step 2/3: Found {len(known_drugs)} drugs from Open Targets")
...
class LiteratureAgent(BaseAgent):
    ...
    def enrich(self, candidate: DrugCandidate, target: str, disease: str,
        ...
        if strategy == "target_focused":
            queries.append(f'"{primary_name}"[Title/Abstract] AND "{target}"[Title/Abstract]')
        elif strategy == "disease_focused":
            queries.append(f'"{primary_name}"[Title/Abstract] AND "{disease}"[Title/Abstract]')
        else:
            queries.append(f'"{primary_name}"[Title/Abstract] AND "{target}"[Title/Abstract]')
            queries.append(f'"{primary_name}"[Title/Abstract] AND "{disease}"[Title/Abstract]')

        queries = queries[:MAX_PUBMED_QUERIES_PER_CANDIDATE]
        ...
        for query in queries:
            if len(queries_tried) >= MAX_PUBMED_QUERIES_PER_CANDIDATE:
                break
        ...
        candidate.provenance.pubmed_pmids = [a.get("pmid") for a in articles if a.get("pmid")]
        candidate.provenance.pubmed_queries_tried = queries_tried

        if articles:
            assessment = self._assess_articles(articles, primary_name, target, disease)
            candidate.literature_support_level = assessment.get("support_level", "none")
            candidate.literature_strength = assessment.get("strength", 0)
            candidate.literature_summary = assessment.get("summary", "")
            candidate.literature_supporting_pmids = assessment.get("supporting_pmids", [])
        else:
            candidate.literature_support_level = "none"
            candidate.literature_strength = 0
            candidate.literature_summary = "No relevant PubMed articles found."
...
class EvaluationAgent(BaseAgent):
    ...
    def _score_candidates(self, candidates: List[DrugCandidate]) -> List[DrugCandidate]:
        ...
        for c in candidates:
            ...
            literature_support = max(0.0, min(10.0, float(c.literature_strength)))

            # safety_profile score
            safety_flags = c.safety_flags or []
            if "BOXED_WARNING" in safety_flags:
                safety_profile = 3
            elif "HIGH_SERIOUS_RATIO" in safety_flags:
                safety_profile = 5
            ...
            disease_relevance = max(0.0, min(10.0, float(c.disease_relevance_score * 10)))

            # Calculate total_score
            c.scores["target_binding"] = target_binding
            c.scores["literature_support"] = literature_support
            c.scores["safety_profile"] = safety_profile
            c.scores["development_stage"] = development_stage
            c.scores["disease_relevance"] = disease_relevance
